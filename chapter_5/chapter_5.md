...
その日の夜、湊は自宅のリビングでGitHub Copilotを試してみた。確かにコードを書くスピードは速くなり、AIが提案するコードをそのまま使えば開発時間は短縮できそうではあった。
...
その日の午後、湊は実際にAIとペアプログラミングを試してみることにした。オフィスの自分のデスクで、山田の提案を受けてGitHub Copilotを使いながら、実際の機能開発を進めてみる。
...
それから数週間、湊はチームで「メモ→タスク」のフローを整え、検収のチェックリストを簡素化した。どのプロジェクトが詰まっているかをボードで見えるようにし、優先して検収するようにした。すると、対応に振り回されず、「考える仕事」に集中できる時間が少しずつ戻ってきた。
...
そのための鍵の一つが**ロードタイム**です。人がプロジェクトを想起し、メモリに展開し、考え始めるまでの時間。検収のロードタイムも同様で、「できました」と言われてから文脈を思い出し成果物を評価できる状態になるまでの時間。これが3秒なのか3分なのかで生産性は桁違いに変わります。メモ→アイデア→タスクの対話的進行、納品→検収→リリースの対話的進行、そしてどのプロジェクトにどれだけAgent時間を投下しているか・どこが詰まっているかの**リソース可視化**で、ロードタイムを短くし、対応荒れを防ぐことができます。

#### AI疲れとコンテキストエンジニアリング

AIを導入したのに「かえって疲れた」と感じる現象を**AI疲れ（AI Fatigue）**といいます。AIは仕事の「密度」を高めます。同じ時間のなかで、意思決定や不確実性への対峙が増えるのです。意思決定は体力（HP）ではなく**精神的なエネルギー（MP）**を消耗させますが、人間のMPはそう簡単には増えません。そのため、これまでと同じ内容の仕事を「AIで速く」こなそうとすると、限界が来ます。また、AIの**自律性が低い**と「AIが5分動く→確認・指示→AIが5分動く…」の繰り返しになり、常に監視と判断が求められ、認知負荷が最大化します。
