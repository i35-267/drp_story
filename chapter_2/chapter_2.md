...
また、ツールを使って定量的データ・定性データを同じ場所に集めることは重要です。
前述したエンジニアリング領域での開発生産性フレークワークやプロジェクトマネジメントとエンジニアリングの接続が可能になります。

国内で言えば、「経営と開発現場をつなぐAI時代の開発資本プラットフォーム」をタグラインとした「**Findy Team+**」が有名なツールとしてあります。
FourKeysの可視化から、AIツールの利用状況、プロジェクトと開発現場との接続も可能になります。

![Findy Team+：開発資本スコアα、サイクルタイム分析、プロジェクト資産分析のダッシュボード画面](assets/chapter_2_findy_team_plus.png)
