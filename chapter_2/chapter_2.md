...
ある記事は「コード行数で測るべき」と主張している。1日あたりのコード行数、1週間あたりのコード行数。でも、湊は首を横に振る。コード行数だけでは、無駄なコードを書けば数字は上がる。リファクタリングでコードを削減すれば数字は下がる。これが生産性の指標として適切なのか。
...
窓の外はまだ暗い。湊はコーヒーを淹れに行きデスクに戻った。オフィスには誰もいない。静まり返った空間で、湊は改めて考える。こういう根本的な問いに向き合うのは初めてだった。これまで湊は「与えられたタスクをこなす」ことに集中していたが、リーダーになって3ヶ月、もっと大きな視点で物事を考える必要があると感じていた。
...
湊は自問する。短期的には機能をリリースできている。でも、長期的に見ると、チームは疲弊している。開発速度は低下している。バグの発生率は増加している。
