...
開発速度は30%向上し、バグ検出率は40%向上していた。技術的負債の返済も進み、チーム全体の疲労度も軽減されていた。これまでのAI活用の取り組みは、開発生産性ツリーの観点から整理すると、戦略1（開発プロセスの高速化）によりIncrement（出口）を1.5倍にし、戦略2（Ops工数のAI置き換え）によりCapacity（入口）を1.5倍にすることで、全体として開発生産性を150%に向上させることができていた。
...
田中部長の言葉に、湊は頷きながら聞いた。確かにエンジニアの視点だけでは不十分かもしれない。これまでに他部署からの評価は得られていた。営業部からは顧客満足度の向上、サポート部からはバグ報告の削減、マーケティング部からはリリース計画の安定化という評価を得ていた。
...
「僕ら事業サイドが求めているのは、開発の速度じゃない。『この見積もりを信じて投資判断していいか』という確信だ。半年の計画を信じて営業戦略を立てられるか。それが全てだ。エンジニアはリードタイムやデプロイ頻度を見ている。でも事業責任者は、計画を信じて立てられるか、意思決定の前提として使えるかを見ている」
...
「あと一つ、開発が追いつかないと言うとき、足りないのは実装リソース、つまり人数なのか、それとも『何を作るか』を決める力なのか。そこを間違えると、人を増やしても忙しいまま進まない。本当のボトルネックがどこにあるか、見極める必要がある」

伊藤部長がそう付け加えると、湊ははっとした。

「本当のボトルネックがどこにあるか、見極める。その視点は持っていませんでした」

「そうだ。でもそれをどう説明するかが重要だ。エンジニアの視点だけで説明しても、経営層には伝わらない。事業視点で説明することで、初めて理解してもらえる」

伊藤部長の言葉に、湊は静かに頷いた。確かにエンジニアの視点だけでなく、事業視点も必要だった。経営会議に向けて、何をどう語るか。その輪郭が少し見えてきた。

### 事業視点の理解

伊藤部長の言葉を噛みしめながら、湊は開発組織と事業側で測っているものが違うこと、計画信頼性が本質だと腹落ちした。

### 解説：エンジニア視点とビジネス視点の融合

この節の論点：エンジニア視点とビジネス視点で「測っているもの」が違うことを理解し、開発生産性の本質が約束の信頼性であることを共有する必要性です。
...
**予算（財務）起点での開発投資の分解**
- 予算から人件費、開発区分、工数、施策、バリューストリーム、開発ライフサイクルへとつながる因果構造
...
「技術的負債は、開発効率の低下リスクとして説明する。どうかな？」

「それだと、まだ抽象的かもしれない。もっと具体的に、コストや時間で説明できないか？」

飛鳥が身を乗り出して加わった。

「リファクタリングは、将来への投資として説明する。これはどう？」

「いいね。でも投資対効果も示した方がいいかもしれない」

山田が補足した。

「コードの品質は、継続的な競争優位性として説明できる。これは事業視点で説明できそうだね」
...
「伊藤部長との話で気づいたんだけど、『人が足りない』と言われたとき、足りないのは人数ではなく、優先順位をつけて『これは作る、これは作らない』と決められる力かもしれない。その視点も、経営層に伝えられるとよいかもしれない」
...
「次に、開発投資がどこに使われているかの構造です。先ほど田中部長とお話ししていた、予算から開発ライフサイクルまで一本でつながる枠組みで整理しています。価値創出工数と価値維持工数の区別で見ると、現在は価値維持に時間を使っている状況でした」

湊はAI戦略の説明に移った。

「これまでのAI活用の取り組みは、開発生産性ツリーの観点から2つの戦略に整理できます。戦略1は、開発プロセスの高速化により、同じ投入工数でより多くの成果物を生み出すことで、Increment（出口）を1.5倍にします。戦略2は、Ops工数をAIに置き換えることで、余剰工数を新規開発に回し、Capacity（入口）を1.5倍にします。両戦略の組み合わせにより、開発生産性を150%に向上させることができます」
...
「聞いた話だけど、部長が事前にCFOとCTOに個別に説明していたらしい。『湊のチームの取り組みを聞いてやってほしい』って」

湊は驚いた。田中部長は何も言っていなかった。数字を出せと追及していたあの部長が、陰で湊を支えていた。

### 解説：PMやPdMと共に創る「共通の価値基準」

この節の論点：技術用語をビジネス用語に翻訳し、開発生産性ツリーで財務と開発行動を接続して、共通の価値基準を創る実践です。
...
「湊君、君が変化の起点になったね。この取り組みが、組織全体に良い影響を与えている。前は進捗会議で数字だけ見てたけど、今は動いているところを見に来るのが楽しみになってる。出てくるものに一貫性が出てきたので任せやすくなった」
...
「湊くん、君が切り開いた道を、他のチームも歩み始めてるね。エンジニアの視点だけでなく、事業視点も持つ。それが重要だったんだよ」
...
この節の論点：ビジネスインパクト・持続可能性・効率性の3軸で多角的に評価する枠組みを設計し、工数・工期・事業KPIを包括した説明を可能にすることです。
...
ストーリーで伊藤部長が語った通り、開発生産性の本質は約束の信頼性にある。この信頼性を評価するためには、多角的な視点が必要です。

#### 内部品質投資の再定義

技術的負債の返済は、単なる「負債の返済」ではなく、「将来の見積精度向上とリードタイム分散低減への投資」として位置づける必要があります。これにより、内部品質投資が事業価値に直接つながることが明確になります。

#### 付加価値生産性と物的生産性——責務とコミット

エンジニアは付加価値生産性（チームの外で決まる価値への貢献）にコミットしつつ、物的生産性（回転数）に責務を持ちます。アジャイルのサイクルを回す原資はこの回転数です。

回転数とは、不確実性に対する生産証明として曝露（Exposure）の回数を増やすことです。定期的に動作するソフトウェアを披露する実践は、この曝露の回数を増やす行為そのものに対応します。短い間隔で動くものを見せることは、物的生産性（回転数）を高めつつ、その先の付加価値にコミットする土台になります。多角的な評価軸（ビジネスインパクト・持続可能性・効率性）を考えるとき、「どれだけ短い間隔で価値検証可能な形で出しているか」も一つの観点として扱えます。
...
個人レベルの生産性向上がそのまま組織・事業の成果に直結するとは限りません。NBERの「Firm Data on AI」では、約70%の企業がAIを使用しているにもかかわらず、80%超が過去3年間で雇用・生産性に影響がなかったと回答しており、個人と組織のギャップが示されています。第5章で触れた**アムダールの法則**を思い出してください。並列化できない部分（要件定義・設計判断・レビュー・承認など）が一定割合残る限り、実装だけを10倍速くしても全体の速度には上限があります。本質的な仕事が30%を占めれば全体は約2.7倍、50%なら約1.8倍程度です。多角的な評価では、この「どこがボトルネックか」を可視化することが有効です。工数・工期に加え「どの工程に時間がかかっているか」をテレメトリで捉える発想（ベラーほかのDiff Authoring Time（DAT）など）は、介入効果の検証や評価軸の設計に有効です。DORAメトリクス（Four Keys：デプロイ頻度・変更リードタイム・変更失敗率・復旧時間）はスピードと安定性の対でフローを捉える最小セットを提供し、事業計画の信頼性との橋渡しに用いることができます。開発者体験の測定では、サーベイに基づく**知覚的指標**と、システムから取れる**ワークフロー指標**を対で見ることが推奨されています（ノダら、*DevEx: What Actually Drives Productivity?*, ACM Queue, Vol. 21, No. 2, 2023）。財務KPIやDORAと並べ、現場の感じ方とログの両方を突き合わせると、ボトルネックの説明がしやすくなります。多角的な評価軸には、こうした知見を組み込むことが有効です。

#### プログラミングとエンジニアリング——ボトルネックの上流化

AIが「How（どう作るか）」を担うほど、人間の仕事は「What（何を作るか）」と「Why（なぜ作るか）」に寄っていきます。**プログラミング**は、既知の解法を正確に実装する営みです。仕様が決まっており、入出力が明確で、正解がある。速さと正確さが価値で、AIが最も得意な領域です。一方、**エンジニアリング**は、未知の問題を試行錯誤で探索する営みです。何を作るべきかが不明確で、作ってみないとわからず、正解がないなかで判断する。仮説、実験、学習のサイクルが中心で、人間の仕事として残る領域です。ボトルネックは上流——顧客の課題の特定、設計判断、暗黙知の形式知化——へ移動します。多角的な評価では、単に「コードを書く速度」だけでなく、この上流での価値創出（問題定義、文脈設計、意思決定の質）をどう測るかも視野に入れることが重要です。
...
第5章で整理したとおり、AIを活用した開発生産性向上の戦略は2つにまとめられます。**戦略1**は開発プロセス（工数・工期）をAIネイティブな進め方で高速化し、Increment（出口）を1.5倍にすること。**戦略2**はOps工数をAIに置き換え（または協働）し、余剰工数を新規開発に回してCapacity（入口）を1.5倍にすること。両戦略の組み合わせで、Capacityに対するIncrementの量を現状100%から150%に向上させられます。詳細は第5章の開発生産性ツリー（Capacity・Increment の整理）を参照してください。
...
**レポート例**
- デプロイ頻度が週1回から1日1回に向上した結果、市場投入速度が30%向上し、売上に10%貢献した事例
- 変更失敗率が20%から10%に削減した結果、サポート対応時間が40%削減し、コスト削減500万円/年につながった事例
...
- 開発速度が30%向上した場合：開発コスト15%削減
- バグ検出率が40%向上した場合：バグ対応工数40%削減
...
本章では、湊がエンジニアの視点だけでなく、PM、PdM、事業責任者の視点も取り入れた新しい生産性評価の枠組みを提案する過程を描きました。伊藤部長が語ったように、開発生産性の本質は約束の信頼性にあり、計画を信じて立てられるようになることが事業側にとっての一番の価値だという理解が、経営会議と新評価制度の設計に貫かれています。
...
- **Noda, A. et al.** (2023). "DevEx: What Actually Drives Productivity?" ACM Queue, Vol. 21, No. 2. 知覚的指標とワークフロー指標の併用。
...
- **NBER / 個人と組織のギャップ**  
  NBER "Firm Data on AI" 等に基づく、個人レベルの生産性向上と組織レベルの成果のギャップに関する知見。IT生産性パラドックスと歴史的教訓（Solow、Brynjolfsson等）については第7章末の参考文献「§7（組織変革・補完的導入）」を参照。

- **石垣雅人.** 「『開発生産性を上げる改善』って儲かるの？に答えられるようにする」. Speaker Deck, 開発生産性カンファレンス2024, 2024/06/28. https://speakerdeck.com/i35_267/is-development-productivity-profitable スライド10「価値創造への主体性と開発生産性」を参照。スライド内で引用されている広木氏「大規模言語モデル時代の開発生産性」に基づく付加価値生産性・物的生産性（回転数）・曝露の議論。
