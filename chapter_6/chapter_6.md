...
第5章で整理したとおり、AIを活用した開発生産性向上の戦略は2つにまとめられます。**戦略1**はValueStream（工数・工期）をAIネイティブなプロセスで高速化し、Increment（出口）を1.5倍にすること。**戦略2**はOps工数をAIに置き換え（または協働）し、余剰工数を新規開発に回してCapacity（入口）を1.5倍にすること。両戦略の組み合わせで、Capacityに対するIncrementの量を現状100%から150%に向上させられます。詳細は第5章およびconfig.mdの開発生産性ツリーを参照してください。
