...
開発チームの現場では、生産性はスループット、すなわち1人月あたりにどれだけ成果物を出せるかをさします。ソフトウェアの内部品質やテストカバレッジ、デプロイ頻度などを含むFour Keysのような指標がここに対応し、エンジニアやデザイナーが日々扱う領域です。

一段上のエンジニアリングマネージャー（EM）やプロジェクトマネージャー（PM）、ディレクター層では、スループットは人月に換算されます。ここでの生産性は、計画やロードマップに対して工数や工期がどう動くかという問いです。生産性が高いほど当初計画より短い工期で済み、見積もりの精度も上がりやすくなります。

さらに上の事業責任者であるプロダクトマネージャー（PdM）やビジネスマネージャー（BM）の層では、人月は金額として読み替えられます。販管費の最適化が焦点になり、同じ単価でも生産性次第で施策の回数や質が変わります。生産性が落ちれば、同じ成果を得るのに追加の投資が要ります。

最上位の経営層では、金額は損益計算書や貸借対照表の語彙へと移ります。損益計算書上は販管費としてどれだけ影響するか、貸借対照表上は資産計上や減価償却、税負担にどう効くかが論点になり、経理部門は貸借対照表側の数値を扱います。

現場がスループットを上げたと言うとき、経営が聞いているのは販管費が下がったのか、資産効率は改善したのか、という問いです。同じ生産性という一語でも、階層ごとに通貨の単位が違います。この変換の連鎖を意識しないまま改善を語ると、現場の努力が経営の言葉に届かず、投資判断や評価のすれ違いが起きます。
