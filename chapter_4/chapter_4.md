...
Faros AI の分析（1,255チーム・10,000名超の開発者テレメトリ）では、AI採用チームでタスク完了数が約21%増・マージPR数が約98%増だった一方、組織レベルでは全社スループット・DORA指標・品質KPIに有意な相関がなく、PRレビュー時間が91%増加・バグ発生率が9%上昇していました。下流のボトルネックが価値を吸収している例です。個人の稼働を高めたりタスク完了数を増やしたりしても、価値が顧客に届くまでの流れが詰まっていれば組織の成果には結びつきません。指標改善が組織成果に届かない理由の一つは、「どれだけ動いたか」ではなく「どれだけ早く届いたか」を見ていないことにあるのです。
