...
指標を改善しても、組織の成果に届かないことがあります。その理由の一つが**消える生産性（Vanishing Productivity）**です。効率化によって生まれた時間や余力が、組織の慣性に吸収され、最終的な価値創造につながらない現象を指します。例えば、空いた時間が価値の低い仕事や形式的な会議に使われたり、チームレベルではタスク完了数・マージPR数が大きく増えているのに、組織レベルでは全社スループット・品質指標に有意な改善が見られなかったりします。Faros AI の分析（1,255チーム・10,000名超の開発者テレメトリ）では、AI高採用チームでタスク完了数が約21%増・マージPR数が約98%増だった一方、組織レベルでは全社スループット・DORA指標・品質KPIに有意な相関がなく、PRレビュー時間が91%増加・バグ発生率が9%上昇していました。下流のボトルネックが価値を吸収している例です。
