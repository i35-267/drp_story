...
### 技術的負債の現実と向き合う普段は別々に食べることが多いが、今日は自然と同じテーブルに座った。
...
Martin Fowlerは、ソフトウェアの品質を「外部品質」と「内部品質」に分類しています。
...
一般的な認識では「高品質なものは高価」というトレードオフがありますが、ソフトウェアの内部品質についてはこの法則が当てはまりません。Martin Fowlerの研究によれば、**高品質なソフトウェアは実際には安く作れる**ことが示されています。
...
湊は改めて考えてみた。開発生産性の本質は「開発組織が『いつまでに何ができるか』と語った約束が、どれだけ信頼できるか」である。技術的負債の返済は、単なる「負債の返済」ではなく、「将来の見積精度向上とリードタイム分散低減への投資」として位置づける必要がある。これにより、内部品質投資が事業価値に直接つながることが明確になる。
...
### チームミーティングでの勇気ある提案プロジェクターに接続し、スライドを開く。5名のチームメンバー全員が湊を見ている。
...
開発生産性の本質は「開発組織が『いつまでに何ができるか』と語った約束が、どれだけ信頼できるか」です。エンジニアとしての説明責任を果たすことは、この信頼性を高めるために不可欠です。
